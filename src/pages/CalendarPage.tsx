...
function getCourseColor(contextCode: string | number): string {
  const num = typeof contextCode === "string" 
    ? parseInt(contextCode.replace(/\D/g, ""), 10) || 0 
    : contextCode;
...
export default function CalendarPage() {
  ...
  const isLoading = !isLoaded || assignments === undefined || courses === undefined;
  ...
  const eventsByDate = useMemo(() => {
    ...
    (assignments ?? []).forEach((a) => {
      if (!a.dueAt) return; // Skip assignments without due dates
      
      const dateStr = a.dueAt.split("T")[0];
      ...
      const ev: CalendarEvent = {
        ...
        context_code: course?.courseCode,
                      ...
                      >
                        ...
                        {dayEvents.slice(0, 2).map((ev, idx) => (
                          <div
                            ...
                            style={{ backgroundColor: getCourseColor(ev.context_code || ev.course_id || "") }}
                          >
                ...
                <div
                  ...
                  onClick={() => {
                    if (isAssignment && ev.assignment) {
                      navigate(`/courses/${ev.assignment.course_id}/assignments/${ev.assignment.id}/coach`);
                    }
                  }}
