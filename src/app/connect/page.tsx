...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  access_denied: "Canvas access was denied. Please authorize StudyHub.",
};
...
function ConnectPageInner() {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      await initUserDexie(userId, data.displayName ?? "Student");
      syncCanvasDataToDexie(userId).catch(() => {});
      syncUserToSupabase(userId, data.displayName ?? "Student").catch(() => {});
  ...
  const handleOAuth = () => {
    window.location.href = "/api/auth/canvas/start";
  };
            ...
            {showHelp && (
              <div className="mt-3 rounded-xl bg-muted p-4 text-left">
                ...
                <ol className="space-y-1 text-xs leading-relaxed text-muted-foreground">
                  ...
                  <li>5. Enter &quot;StudyHub&quot; and click <strong>Generate Token</strong></li>
...
async function initUserDexie(userId: string, displayName: string) {
  const existing = await db.users.get(userId);
  if (!existing) {
    await db.users.put({ id: userId, displayName, updatedAt: Date.now() });
  }
