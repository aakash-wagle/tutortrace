...
export default function StudyPage({
  ...
  const { data: deck, isLoading } = useSWR<DeckData>(`/api/flashcards/${deckId}`, fetcher);
  ...
  const orderedCards = cardOrder
    ? cardOrder.map((i) => cards[i]).filter(Boolean)
    : cards;
  ...
  const markDifficulty = async (difficulty: string) => {
    ...
    await fetch(`/api/flashcards/${deckId}/cards/${currentCard.id}`, {
      method: "PATCH",
